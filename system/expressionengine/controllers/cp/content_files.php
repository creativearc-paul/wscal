...
class Content_files extends CP_Controller {
	...
	public function index()
	{
		...
		$this->table->set_columns(array(
			...
			'file_name'		=> array(),
            'thumbnail'     => array('header' => 'Thumbnail','sort' => FALSE), // GHC added
	...
	private function _fetch_file_list($files, $total_filtered)
	{
		...
		if ($total_filtered > 0 AND ! empty($this->_upload_dirs))
		{
			...
			while ($file = array_shift($files))
			{
                ...
                //////////////////////////
                // GHC modifications start
                //////////////////////////
                if($file['mime_type'] == 'image/jpeg' || $file['mime_type'] == 'image/gif' || $file['mime_type'] == 'image/png'){
                    $r['thumbnail'] = '<img src="' . $this->_upload_dirs[$file['upload_location_id']]['url'] . '_thumbs/' . $file['file_name'] . '" >';
                    $dims = @explode(' ', $file['file_hw_original']);
                    $r['thumbnail'] .= ($dims[1] != '' && $dims[0] != '' ) ? '<br>' . $dims[1] . 'px X ' . $dims[0] . 'px' : '';
                } else {
                    $r['thumbnail'] = '';
                }             
                //////////////////////////
                // GHC modifications stop
                //////////////////////////
