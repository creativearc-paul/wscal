...
class Structure_entries{
    ...
    function __construct(){
        ...
        $tempIndexArray = explode("/", str_replace(array(
            "http://",
            "https://",
        ...
        if(!isset(ee()->session->cache)){
            ee()->session->cache = array();
        }
        if(!isset(ee()->session->cache['structure_entries'])){
            ee()->session->cache['structure_entries'] = array();
        }

        //###   Get the upload folder paths   ###
        //		ee()->load->library('typography');
        //		ee()->typography->initialize();
        //		$folderPaths = ee()->typography->file_paths;
        ...
        $param = html_entity_decode(ee()->TMPL->fetch_param('parent'));
        if($param == "current"){
            $parentURL = ee()->functions->fetch_current_uri();
        } else {
            if(substr($param, 0, 1) == "/" || substr($param, 0, 7) == "http://"){
                $parentURL = $param;
        ...
        //###   category ID   ###
        $param = ee()->TMPL->fetch_param('category_id');
        ...
        //###   Child/Parent Depth   ###
        $param = ee()->TMPL->fetch_param('depth');
        ...
        $param = ee()->TMPL->fetch_param('limit');
        $limit = preg_replace("/[^0-9]/", '', $param);

        //###   Check if Status is restricted   ###
        $param = ee()->TMPL->fetch_param('status');
        $param = ee()->db->escape_str($param);
        $restrictStatus = ee()->security->xss_clean($param);

        //###   Debug!   ###
        $debug = FALSE;
        if($param = ee()->TMPL->fetch_param('debug')){
        ...
        $random = FALSE;
        if($param = ee()->TMPL->fetch_param('random')){
            if(strtolower($param) === "true" || strtolower($param) === "t" || strtolower($param) === "yes" || strtolower($param) === "y" || strtolower($param) === "1"){
                $random = TRUE;
            }
        }
        ...
        $convertHTML = FALSE;
        if($param = ee()->TMPL->fetch_param('convert_html')){
            if(strtolower($param) === "true" || strtolower($param) === "t" || strtolower($param) === "yes" || strtolower($param) === "y" || strtolower($param) === "1"){
                $convertHTML = TRUE;
            }
        }
        ...
        $matchParent = FALSE;
        if($param = ee()->TMPL->fetch_param('include_parents')){
            if(strtolower($param) === "true" || strtolower($param) === "t" || strtolower($param) === "yes" || strtolower($param) === "y" || strtolower($param) === "1"){
                $matchParent = TRUE;
            }
        }
        if($param = ee()->TMPL->fetch_param('include_parent')){
            if(strtolower($param) === "true" || strtolower($param) === "t" || strtolower($param) === "yes" || strtolower($param) === "y" || strtolower($param) === "1"){
                $matchParent = TRUE;
            }
        }
        ...
        //###   Debug Output   ###
        ee()->TMPL->log_item("structure_entries: Starting main loop through entries. Total rows from DB=" . $sqlStructure->num_rows());
        ...
        foreach($sqlStructure->result_array() as $structureField){
            ...
            if(!empty($entry_id) && ($status != "closed" || $restrictStatus == "closed")){
                ...
                if(isset(ee()->session->cache['structure_entries']['entryData' . $entry_id])){
                    $sqlEntryResult = ee()->session->cache['structure_entries']['entryData' . $entry_id];
                } else {
                    $sql = "SELECT exp_channel_data.*, exp_channel_titles.*, exp_channels.channel_name
							 ...
							 LIMIT 1";
                    $sqlEntryResult = ee()->db->query($sql);
                    ee()->session->cache['structure_entries']['entryData' . $entry_id] = $sqlEntryResult;
                }
                ...
                if($match){
                    ...
                    //###   Debug Output   ###
                    ee()->TMPL->log_item("structure_entries: tagdata=" . ee()->TMPL->tagdata);

                    //###   Parse Matrix Fields   ###
                    if(!empty($Matrix)){
                        //###   Legacy Matrix requires this to be unset   ###
                        unset(ee()->extensions->last_call);

                        //###   Legacy Matrix required third parameter (from v2.1.4 or thereabouts it doesn't)   ###
                        $output = $Matrix->channel_entries_tagdata($output, array(
                            "entry_id"      => $entry_id,
                            "entry_site_id" => $site_id,
                        ), $EntryDetails[0]["channel_name"]);
                    }
                    ...
                    } //###   End of foreach (ee()->TMPL->var_pair)

                    //###   Debug Output   ###
                    ee()->TMPL->log_item("structure_entries: ID=" . $entry_id . " url=" . $entryURL);

                    //###   Loop through all the single variables from template   ###
                    foreach(ee()->TMPL->var_single as $key => $fieldName){
                        if($fieldName == "page_uri"){
                            //###   This is the full URL to (and including) the page
                            $output = ee()->TMPL->swap_var_single($fieldName, $entryURL, $output);

                        } else {
                            if($fieldName == "channel" || $fieldName == "channel_title"){
                                $output = ee()->TMPL->swap_var_single($fieldName, $EntryDetails[0]["channel_name"], $output);

                            } else {
                                if($fieldName == "depth"){
                                    $output = ee()->TMPL->swap_var_single($fieldName, $depthCount, $output);

                                } else {
                                    if($fieldName == "count" || $fieldName == "counter"){
                                        $output = ee()->TMPL->swap_var_single($fieldName, $matchCounter, $output);

                                    } else {
                                        if($fieldName == "parent_id"){
                                            $output = ee()->TMPL->swap_var_single($fieldName, $thisParentID, $output);

                                        } else {
                                            if($fieldName == "children_total" || $fieldName == "total_children"){
                                                $output = ee()->TMPL->swap_var_single($fieldName, $children, $output);

                                            } else {
                                                if($fieldName == "sibling_total" || $fieldName == "siblings_total" || $fieldName == "total_siblings" || $fieldName == "total_sibling"){
                                                    $output = ee()->TMPL->swap_var_single($fieldName, $siblings, $output);

                                                } else {
                                                    if($fieldName == "sibling_count"){
                                                        $output = ee()->TMPL->swap_var_single($fieldName, $siblingCounter[$thisParentID], $output);

                                                    } else {
                                                        if($fieldName == "last_sibling" || $fieldName == "last_child"){
                                                            if($siblingCounter[$thisParentID] == $siblings){
                                                                $output = ee()->TMPL->swap_var_single($fieldName, 1, $output);
                                                            } else {
                                                                $output = ee()->TMPL->swap_var_single($fieldName, 0, $output);
                                                            }

                                                        } else {
                                                            if($fieldName == "page_url"){
                                                                //###   This is the individual segment URL (not the full URL)
                                                                $output = ee()->TMPL->swap_var_single($fieldName, $slug, $output);

                                                            } else {
                                                                if(preg_match("/^switch\s*=.+/i", $fieldName)){
                                                                    $varParam = ee()->functions->assign_parameters($fieldName);
                                                                    ...
                                                                    $output = ee()->TMPL->swap_var_single($fieldName, $switchOption, $output);
                                                                    ...
                                                                    if(isset($EntryDetails[0][$fieldName])){
                                                                        ...
                                                                        //							$output = ee()->TMPL->swap_var_single($fieldName, $newContent, $output);
                                                                        $output = ee()->TMPL->parse_variables_row($output, array($fieldName => $newContent));
                                                                        //TO DO: Parse variables correctly with EE, including format parameter
                                                                        /*							$variables = array();
                                                                                                    $variables[] = array($fieldName => $newContent);
                                                                                                    $output = ee()->TMPL->parse_variables($output, $variables); */
                                                                    }
                                                                }
                                                            }
                                                        }
                        ...
                        if($fieldName == "current_parent"){
                            //###   Debug Output   ###
                            ee()->TMPL->log_item("structure_entries: current_parent variable: Actual URL=" . $compareIndex . "/" . ee()->uri->uri_string() . " Structure page_uri=" . $entryURL);

                            if(strpos($compareIndex . "/" . ee()->uri->uri_string(), $entryURL) !== FALSE && $entryURL !== $compareIndex . "/" . ee()->uri->uri_string() && $entryURL !== $compareIndex && $entryURL !== "/"){
                                $output = ee()->TMPL->swap_var_single($fieldName, 1, $output);
                            } else {
                                $output = ee()->TMPL->swap_var_single($fieldName, 0, $output);
                            }
                        ...
                        if($fieldName == "parent_active"){
                            //echo "#".$compareIndex . ee()->uri->uri_string() ." ~ ". $entryURL."#<br />";
                            ...
                            //###   Debug Output   ###
                            ee()->TMPL->log_item("structure_entries: parent_active variable: Actual URL=[" . $compareIndex . "/" . ee()->uri->uri_string() . "] parentURL=[" . $parentURL . "] entryURL=[" . $entryURL . "]");
                            if(!empty($parentURL)){
                                if(strpos($compareIndex . "/" . ee()->uri->uri_string(), $parentURL) !== FALSE && $entryURL !== $compareIndex . "/" . ee()->uri->uri_string() && $entryURL !== "/"){
                                    $output = ee()->TMPL->swap_var_single($fieldName, 1, $output);
                                } else {
                                    $output = ee()->TMPL->swap_var_single($fieldName, 0, $output);
                                }
                        ...
                        if($fieldName == "current_page"){
                            //###   Debug Output   ###
                            ee()->TMPL->log_item("structure_entries: current_page variable: Actual URL=[" . $siteIndex . "/" . ee()->uri->uri_string() . "] Structure page_uri=[" . $entryURL . "] page_url=[" . $slug . "] Structure slug=" . $Structure->page_slug());
                            // TO DO - NOT CORRECTLY COMPARING PAGE - NEEDS FULL URI
                            //POSSIBLE FIX - NEEDS TESTING ON NORMAL WEBSITE:

                            //							if ($slug != $Structure->page_slug() || ($slug == "" && $siteIndex.ee()->uri->uri_string() != "")) {
                            //								$output = ee()->TMPL->swap_var_single($fieldName, 0, $output);
                            if($slug == $Structure->page_slug() && ee()->uri->segment(2) === FALSE){
                                //###   Page URL (Slug) is identical, and at root level in URL (no segment 2)   ###
                                $output = ee()->TMPL->swap_var_single($fieldName, 1, $output);
                            } else {
                                if($siteIndex . "/" . ee()->uri->uri_string() . "/" == $entryURL){
                                    //###   Full URL match   ###
                                    $output = ee()->TMPL->swap_var_single($fieldName, 1, $output);
                                } else {
                                    $output = ee()->TMPL->swap_var_single($fieldName, 0, $output);
                            ...
                            //$output = "#start#".$siteIndex."/".ee()->uri->uri_string()."#".$entryURL."#end#";
                        }
                        if($fieldName == "current_sibling"){
                            ...
                            $compareURL = explode("/", $compareIndex . ee()->uri->uri_string());
                            ...
                            //$output = ee()->TMPL->swap_var_single($fieldName, "~".$parentURL."~".$compareURL, $output);
                        }

                        //###   Replace any Category variables?   ###
                        if(substr($fieldName, 0, 3) === "cat"){
                            //###   Get Associated Categories   ###
                            if(isset(ee()->session->cache['structure_entries']['catLookup' . $entry_id])){
                                $sqlCatLookup = ee()->session->cache['structure_entries']['catLookup' . $entry_id];
                            } else {
                                $sql = "SELECT cat_id
										  FROM exp_category_posts
										 WHERE entry_id = " . $entry_id . "
										 LIMIT 1";
                                $sqlCatLookup = ee()->db->query($sql);
                                ee()->session->cache['structure_entries']['catLookup' . $entry_id] = $sqlCatLookup;
                            }
                                ...
                                if($sqlCategories->num_rows() > 0){
                                    ...
                                    foreach($categories[0] as $catField => $catValue){
                                        ...
                                        if($fieldName === $catField || $AlternativeName === $catField){
                                            $output = ee()->TMPL->swap_var_single($fieldName, htmlspecialchars($catValue), $output);
                                            break;
                                        }
                                    }
                                } else {
                    ...
                    } //###   End of foreach (ee()->TMPL->var_single)
        ...
        $output = ee()->typography->parse_file_paths($output);
    ...
    */
    ...
    private function get_site_pages(){
        ...
        if(isset(ee()->session->cache['structure_entries']['site_pages' . $site_id])){
            $site_pages = ee()->session->cache['structure_entries']['site_pages' . $site_id];
        } else {
            ...
            ee()->session->cache['structure_entries']['site_pages' . $site_id] = $site_pages;

            if(empty($site_pages)){
                //###   Debug Output   ###
                ee()->TMPL->log_item("structure_entries: No pages are currently defined in Structure, so nothing to output!");
        ...
        $SQLResult = ee()->db->query("SELECT module_name
								   FROM exp_modules
        ...
        if(!$StructureEnabled){
            //###   Debug Output   ###
            ee()->TMPL->log_item("structure_entries: Structure is not installed - this plugin extends and requires Structure");
    ...
    private function get_structure_data(){
        ...
        //		if (isset(ee()->session->cache['structure_entries']['structure_data'.$site_id])) {
        //			$sqlStructure = ee()->session->cache['structure_entries']['structure_data'.$site_id];
        ...
        $sql = "SELECT node.*,
				...
				INNER JOIN exp_channel_titles AS expt
				  ...
				  ORDER BY node.lft";
        $sqlStructure = ee()->db->query($sql);
        if($sqlStructure->num_rows() < 0){
            //###   Debug Output   ###
            ee()->TMPL->log_item("structure_entries: No Structure data was returned - check Structure is installed correctly");

            return FALSE;
        }

        //###   Save result to page-load Session   ###
        //			ee()->session->cache['structure_entries']['structure_data'.$site_id] = $sqlStructure;
        //		}
    ...
    private function getClosestPage($entry_id = 0, $direction){
        ...
        $param = ee()->TMPL->fetch_param('entry_id');
        if(empty($param)){
            $param = $entry_id;
        }
        ...
        $param = ee()->TMPL->fetch_param('page_uri');
        $param = ee()->db->escape_str($param);
        $currentPageURI = ee()->security->xss_clean($param);
        ...
        $actualURI = $siteIndex . "/" . ee()->uri->uri_string();
        ...
        if(empty($currentPageURI)){
            //###   Debug Output   ###
            ee()->TMPL->log_item("structure_entries (Next): No page specified and no URI found?!?: Actual URL=" . $actualURI);
        ...
        //###   Debug Output   ###
        ee()->TMPL->log_item("structure_entries (Next): Starting main loop through entries. Total rows from DB=" . $sqlStructure->num_rows());
        ...
        foreach($sqlStructure->result_array() as $structureField){
            ...
            if(!empty($entry_id) && ($status != "closed" || $restrictStatus == "closed")){
                ...
                //###   Debug Output   ###
                ee()->TMPL->log_item("structure_entries (Next): entryURL=" . $entryURL . " actualURI=" . $actualURI);
                ...
                if($entryURL == $actualURI){
                    ...
                    if($direction == "previous" || $direction == "prev"){
                        ...
                        //###   Debug Output   ###
                        ee()->TMPL->log_item("structure_entries (previous): tagdata=" . ee()->TMPL->tagdata);
                        ...
                        foreach(ee()->TMPL->var_single as $key => $fieldName){
                            if($fieldName == "page_uri"){
                                //###   This is the full URL to (and including) the page
                                $output = ee()->TMPL->swap_var_single($fieldName, $previousPageData["entryURL"], $output);

                            } else {
                                if($fieldName == "page_url"){
                                    //###   This is the individual segment URL (not the full URL)
                                    $output = ee()->TMPL->swap_var_single($fieldName, $previousPageData["page_slug"], $output);
                    ...
                    if($foundMatch){
                        ...
                        //###   Debug Output   ###
                        ee()->TMPL->log_item("structure_entries (next): tagdata=" . ee()->TMPL->tagdata);
                        ...
                        foreach(ee()->TMPL->var_single as $key => $fieldName){
                            if($fieldName == "page_uri"){
                                ...
                                if($fieldName == "page_url"){
                                    //###   This is the individual segment URL (not the full URL)
                                    $output = ee()->TMPL->swap_var_single($fieldName, $Structure->page_slug($entry_id), $output);
    ...
    function get_parent($entry_id){
        ...
        if(!empty(ee()->session->cache['structure_entries']['parent' . $entry_id])){
            return ee()->session->cache['structure_entries']['parent' . $entry_id];
        }
        ...
        $sql = "SELECT *
				...
				WHERE entry_id = $entry_id
				LIMIT 1";
        $sqlStructureParent = ee()->db->query($sql);
        if($sqlStructureParent->num_rows() > 0){
            ...
            ee()->session->cache['structure_entries']['parent' . $entry_id] = $parentID;
        } else {
            return;
        }
    ...
    function within_category($entry_id, $catID){
        ...
        $match = FALSE;
        if(isset(ee()->session->cache['structure_entries']['catLookup' . $entry_id])){
            $catLookup = ee()->session->cache['structure_entries']['catLookup' . $entry_id];
        } else {
            $sql = "SELECT cat_id
					  FROM exp_category_posts
					 WHERE entry_id = $entry_id";
            $catLookup = ee()->db->query($sql);
            ee()->session->cache['structure_entries']['catLookup' . $entry_id] = $catLookup;
        }
    ...
    public static function usage(){
        ...
        <ul class="drop-items level{depth}">
            {/if}
            <li {if {current_parent}}class="current" {
            /if}{if {current_page}}class="active"{/if}>
