...
<div class="assets-settingheader">
	<label>
		...
		<?php echo form_dropdown('assets[view]', array('thumbs'=>lang('thumbnails'), 'list'=>lang('list')), $settings['view'],
			'onchange="jQuery(this).parent().parent().next().children(\'.assets-options-\'+this.value).show().siblings().hide();jQuery(this).parent().parent().next().children(\'.assets-options-\'+this.value).find(\'.editable input\').attr(\'disabled\', false);"') ?>
	</label>
</div>
...
<div>
	...
	<div class="assets-options-list"<?php if ($settings['view'] != 'list'): ?> style="display: none"<?php endif ?>>
		...
		<label class="assets-checkbox editable"><?php echo form_checkbox('assets[show_cols][]', 'folder', in_array('folder', $settings['show_cols'])).NBS.NBS.lang('folder') ?></label><br/>
		<label class="assets-checkbox editable"><?php echo form_checkbox('assets[show_cols][]', 'date',   in_array('date',   $settings['show_cols'])).NBS.NBS.lang('date') ?></label><br/>
		<label class="assets-checkbox editable"><?php echo form_checkbox('assets[show_cols][]', 'size',   in_array('size',   $settings['show_cols'])).NBS.NBS.lang('size') ?></label><br/>
	</div>
</div>
