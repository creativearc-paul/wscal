 ...
 */
abstract class Neoseo_entity_abstract{
    ...
    public function __construct(){
        ...
        foreach($this->_form_selects as $field_name){
            $this->SelectOptions->$field_name = ee()->{$this::MOD_NAME . '_form_helper'}->get_select_options($field_name);
        }
        
        // create checkboxes
        $this->Checkboxes = new stdClass();
        foreach($this->_form_checkboxes as $field_name){
            $this->Checkboxes->$field_name = ee()->{$this::MOD_NAME . '_form_helper'}->get_checkboxes($field_name);
        }
...
/*        // get filter presets if passed in url
        ...
        if($this->_listing_filters){
            ...
            if(isset($this->_listing_filters['select_filters'])){
                foreach($this->_listing_filters['select_filters'] AS $SelectFilter){
                    ...
                    //$Filter->selected = (ee()->input->post($SelectFilter->column, TRUE) && array_key_exists(ee()->input->post($SelectFilter->column, TRUE), $SelectFilter->options)) ? ee()->input->post($SelectFilter->column, TRUE) : key($SelectFilter->options); // if not passed, use first
    ...
    public function action_save() {
        ...
        // now validate form
        $rules = ee()->config->item($this::MOD_NAME . '_validation_rules');
        ...
        foreach($RecordModel as $field=>$value) {
            if(isset($_POST[$field])){
                ...
                if(in_array($field, $this->_default_zero_fields)){
                    $RecordModel->$field = 0;
                }
            }
        }
    ...
    public function _get_rmdl($pk = NULL) {
        ...
        try{
            ...
            if(is_null($pk)){
                foreach($this->_implode_fields as $field){
                    $RecordModel->{$field} = explode(',', $RecordModel->$field);
                }
                return $RecordModel;
            }
